...
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"function":"RegisterWizard","Args":["Harry Potter", "British"]}' >&log.txt
 { set +x; } 2>/dev/null
cat log.txt
sleep 3
...
set -x
peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["ReadWizard","Harry Potter"]}' >&log.txt
 { set +x; } 2>/dev/null
cat log.txt
